...
class SACAgent(BaseAgent):
    ...
    def update_critic(self, ob_no, ac_na, next_ob_no, re_n, terminal_n):
        ...
        with torch.no_grad():
            ...
            next_log_probs = next_ac_dist.log_prob(next_ac)
            next_log_probs = next_log_probs.sum(dim = -1, keepdim = True)
            q1, q2 = self.critic_target(next_ob_no, next_ac)
            target : torch.Tensor = re_n + self.gamma*(1-terminal_n)*(torch.min(q1,q2) - self.actor.alpha*next_log_probs)
            target = target.detach()

        pred_q1, pred_q2 = self.critic(ob_no, ac_na)
        critic_loss : torch.Tensor = self.critic.loss(target, pred_q1) + self.critic.loss(target, pred_q2)
