...
class MPCPolicy(BasePolicy):
    ...
    def sample_action_sequences(self, num_sequences, horizon, obs=None):
        if self.sample_strategy == 'random' \
            ...
            elite_reward_idx = np.argsort(predicted_rewards)[-self.cem_num_elites:]
            elite_action_sequences = candidate_action_sequences[elite_reward_idx]
            ...
            for i in range(self.cem_iterations):
                ...
                candidate_action_sequences = np.random.normal(elite_mean, elite_variance, size=(num_sequences, horizon, self.ac_dim))
                predicted_rewards = self.evaluate_candidate_sequences(candidate_action_sequences, obs)
                elite_reward_idx = np.argsort(predicted_rewards)[-self.cem_num_elites:]
                elite_action_sequences = candidate_action_sequences[elite_reward_idx]
                elite_mean = self.cem_alpha*np.mean(elite_action_sequences, axis=0) + (1-self.cem_alpha)*elite_mean
                elite_variance = self.cem_alpha*np.var(elite_action_sequences, axis=0) + (1-self.cem_alpha)*elite_variance

            # TODO(Q5): Set `cem_action` to the appropriate action chosen by CEM
            cem_action = np.random.normal(elite_mean, elite_variance, size=(1, horizon, self.ac_dim))
    ...
    def evaluate_candidate_sequences(self, candidate_action_sequences, obs):
        ...
        sum_of_rewards_per_model = np.empty((0, self.N))
        for model in self.dyn_models: 
            sum_of_rewards = self.calculate_sum_of_rewards(obs, candidate_action_sequences, model)
            sum_of_rewards_per_model = np.append(sum_of_rewards_per_model,sum_of_rewards, axis=0)
        return np.mean(sum_of_rewards_per_model, axis=0)
    ...
    def calculate_sum_of_rewards(self, obs, candidate_action_sequences, model):
        ...
        for t in range(H):
            ...
            rewards[:,t] = self.env.get_reward(x, actions)[0]
            x = model.get_prediction(x, actions, self.data_statistics)
        sum_of_rewards = np.sum(rewards, axis=1)
        return sum_of_rewards.reshape((-1, N))
